...
WavePCMFile::WavePCMFile(const QString & name, const QAudioFormat & format_, QObject *parent_)
    : QFile(name, parent_), format(format_)
{
}

bool WavePCMFile::hasSupportedFormat()
{
...
void WavePCMFile::write(QByteArray arr)
{
    QDataStream out(this);
    out << arr;
}
